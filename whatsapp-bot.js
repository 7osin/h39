...
const startSock = async () => {
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    const senderNumber = senderJid.replace(/@.*$/, '').replace(/^966/, '0'); // رقم سعودي بصيغة 05...

    // إذا أول مرة يرسل هذا الرقم، أرسل له ترحيب
    if (!greetedNumbers.has(senderNumber)) {
      greetedNumbers.add(senderNumber);
      await sock.sendMessage(senderJid, {
        text: `مرحباً بك في   👋\nلمعرفة موعد استلامك أرسل كلمة: استلامي`
      });
      // لا ترد بأي شيء آخر في أول رسالة
      return;
    }

    // إذا أرسل "استلامي" أو "موعدي" أو غيرها
    ...
    if (triggers.includes(cleanText)) {
      ...
      for (const report of reportsLog) {
        found = report.entries.find(e =>
          e.number.replace(/\D/g, '').endsWith(senderNumber.replace(/\D/g, '').slice(-9))
        );
        if (found) break;
      }
      if (found) {
  ...
  sock.ev.on('connection.update', async ({ connection, lastDisconnect, qr }) => {
    if (qr) {
      ...
      qrcode.default.generate(qr, { small: true }) // عرض QR في الطرفية
    }
...
async function downloadPDF() {
  ...
  toggleVisibility(false);
    ...
    document.getElementById("sendReportBtn").onclick = async function() {
      ...
      if (rows.length === 0) return alert("يجب إضافة بيانات أولاً!");
...
function schedulePreReminder(entry, jid, minutesBefore = 30) {
  let [hour, period] = entry.time_receive.split(' ');
  hour = parseInt(hour);
  if (isNaN(hour)) return;
  if (period === 'م' && hour < 12) hour += 12;
  ...
  setTimeout(async () => {
    try {
      await sock.sendMessage(jid, { text: `تنبيه: تبقى ${minutesBefore} دقيقة على موعد استلامك (${entry.time_receive}) في الموقع: ${entry.place}` });
    } catch (e) { }
  }, msUntil);
}
